<?php
 ...
 /**
  * Videos Data Access Object (DAO).
  * This class contains all database handling that is needed to 
  * permanently store and retrieve Videos object instances. 
  */
...
class VideosDao {
     ...
     */
    function getObject(&$conn, $idVideo) {

          $valueObject = $this->createValueObject();
          $valueObject->setIdVideo($idVideo);
    ...
    function load(&$conn, &$valueObject) {
          ...
          if (!$valueObject->getIdVideo()) {
               //print "Can not select without Primary-Key!";
               return false;
          }

          $sql = "SELECT * FROM videos WHERE (idVideo = ".$valueObject->getIdVideo().") "; 
    ...
    function create(&$conn, &$valueObject) {
          ...
          $sql = "INSERT INTO videos ( idVideo, video, url) VALUES (".$valueObject->getIdVideo().", ";
          $sql = $sql."'".$valueObject->getVideo()."', ";
          $sql = $sql."'".$valueObject->getUrl()."') ";
    ...
    function save(&$conn, &$valueObject) {
          ...
          $sql = "UPDATE videos SET video = '".$valueObject->getVideo()."', ";
          $sql = $sql."url = '".$valueObject->getUrl()."'";
          $sql = $sql." WHERE (idVideo = ".$valueObject->getIdVideo().") ";
          $result = $this->databaseUpdate($conn, $sql);
    ...
    function delete(&$conn, &$valueObject) {
          ...
          $sql = "DELETE FROM videos WHERE (idVideo = ".$valueObject->getIdVideo().") ";
          $result = $this->databaseUpdate($conn, $sql);
    ...
    function searchMatching(&$conn, &$valueObject) {
          ...
          if ($valueObject->getIdVideo() != 0) {
              if ($first) { $first = false; }
              $sql = $sql."AND idVideo = ".$valueObject->getIdVideo()." ";
          }

          if ($valueObject->getVideo() != "") {
              if ($first) { $first = false; }
              $sql = $sql."AND video LIKE '".$valueObject->getVideo()."%' ";
          }

          if ($valueObject->getUrl() != "") {
              if ($first) { $first = false; }
              $sql = $sql."AND url LIKE '".$valueObject->getUrl()."%' ";
          }


          $sql = $sql."ORDER BY idVideo ASC ";
    ...
    function singleQuery(&$conn, &$sql, &$valueObject) {
          ...
          if ($row = $conn->nextRow($result)) {
                   ...
                   $valueObject->setIdVideo($row[0]); 
                   $valueObject->setVideo($row[1]); 
                   $valueObject->setUrl($row[2]); 
          } else {
